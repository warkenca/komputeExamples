---
title: "kompute algorithm test - CC data"
...
```{r}
allpheno = allpheno %>% 
  filter(procedure_name=="Clinical Chemistry") %>%
  mutate(proc_short_name=recode(procedure_name, "Clinical Chemistry"="CC")) %>%
...
col_fun(seq(0, nmax))
pdf("~/Google Drive Miami/Miami_IMPC/output/measured_phenotypes_controls_CC.pdf", width = 12, height = 14)
...
pdf("~/Google Drive Miami/Miami_IMPC/output/genetic_corr_btw_phenotypes_Pearson_CC.pdf", width = 5.7, height = 3)
ht = Heatmap(ap.cor, show_column_names = F, row_names_gp = gpar(fontsize = 9), name="Corr")
draw(ht)
dev.off()

pdf("~/Google Drive Miami/Miami_IMPC/output/genetic_corr_btw_phenotypes_rankZ_CC.pdf", width = 5.7, height = 3)
ht = Heatmap(ap.cor.rank, show_column_names = F, row_names_gp = gpar(fontsize = 9), name="Corr")
draw(ht)
dev.off()

pdf("~/Google Drive Miami/Miami_IMPC/output/genetic_corr_btw_phenotypes_ComBat_Adjusted_CC.pdf", width = 5.7, height = 3)
...
ap.stat = ap.stat %>% 
  ...
  filter(procedure_name == "Clinical Chemistry") %>%
  mutate(procedure_name=recode(procedure_name, "Clinical Chemistry"="CC")) %>%
...
length(unique(ap.stat$marker_symbol)) #3983
length(unique(ap.stat$allele_symbol)) #4152
length(unique(ap.stat$proc_param_name)) #27  # number of phenotypes in association statistics data set
length(unique(allpheno$proc_param_name)) #19 # number of phenotypes in final control data
...
if(FALSE){
pdf("~/Google Drive Miami/Miami_IMPC/output/missing_tests_after_filtering_CC.pdf", width = 6, height = 2.7)
ht = Heatmap(t(id.mat), 
...
ht = Heatmap(ap.cor.rank, cluster_rows = TRUE, cluster_columns = TRUE, show_column_names = F, #col = col_fun,
              show_row_dend = F, show_column_dend = F,  # do not show dendrogram
...
if(FALSE){
pdf("~/Google Drive Miami/Miami_IMPC/output/comp_pheno_corr_gene_corr_CC.pdf", width = 10, height = 2.5)
...
g <- ggplot(imp.sub, aes(x=imp.z, y=org.z, col=info)) +
    ...
    theme_minimal()
g
filename <- "~/Google Drive Miami/Miami_IMPC/output/realdata_measured_vs_imputed_info_CC.pdf"
